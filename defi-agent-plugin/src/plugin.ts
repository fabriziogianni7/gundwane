...
const plugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    async function getWalletAddress(ctx: ToolContext): Promise<Address | null> {
      ...
      if (!backendUrl) {
        log("getWalletAddress skipped: backendUrl not set peerId=%s", peerId);
        return null;
      }
      const url = `${backendUrl}/api/wallet/${encodeURIComponent(peerId)}`;
      log("getWalletAddress peerId=%s url=%s", peerId, url);
      try {
        const res = await fetch(url);
        const data = (await res.json()) as { address?: string; error?: string };
        const gotAddress = !data.error && !!data.address;
        if (gotAddress) {
          log("getWalletAddress peerId=%s status=%d address=%s...%s", peerId, res.status, (data.address as string).slice(0, 8), (data.address as string).slice(-6));
        } else {
          log("getWalletAddress peerId=%s status=%d address=%s error=%s", peerId, res.status, data.address ?? "null", data.error ?? "none");
        }
        if (!res.ok) return null;
        if (data.error || !data.address) return null;
        return data.address as Address;
      } catch (err) {
        log("getWalletAddress peerId=%s fetch failed:", peerId, err);
        return null;
      }
    }
    ...
    api.registerTool((ctx: ToolContext) => ({
      ...
      description:
        "Resolve the Telegram user's wallet address from the backend (Privy). Returns address when the user has completed setup in the Mini App; otherwise returns ok: false with a hint for the user.",
      ...
      execute: withErrors(async () => {
        ...
        if (address) {
          return {
            address,
            ok: true,
            message: "Wallet is linked. Use this address for balances, delegation checks, and transactions.",
          };
        }
        return {
          address: null,
    ...
    api.registerTool((ctx: ToolContext) => ({
      ...
      execute: withErrors(async () => {
        ...
        const chainEntries = Object.entries(chains).filter(([id]) => id !== "11155111"); // skip testnet
        const results = await Promise.allSettled(
          chainEntries.map(async ([chainId]) => {
        ...
        for (const c of chainResults) {
          totalNativeEth += parseFloat(c.nativeBalance) || 0;
        }
    ...
    log("register() done; 12 tools registered");
  },
};
