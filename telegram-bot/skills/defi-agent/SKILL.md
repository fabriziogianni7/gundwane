...
| Tool | Purpose |
|------|---------|
| **defi_get_wallet** | Get the Telegram user's wallet address (from backend/Privy). Returns `{ address, ok }`. If `address` is null, direct user to Mini App. |
| **defi_get_portfolio** | Get multi-chain portfolio: native + ERC-20 token balances across ALL configured chains. No parameters needed. Use this for any balance query — it's chain-abstracted. |
...
| **defi_send_transaction** | Execute one call from the user's delegated wallet. For LI.FI `transactionRequest` (to, value, data). Always pass gasLimit. |
| **defi_approve_and_send** | Approve token + execute transaction atomically. For LI.FI ERC-20 swaps. |
...
4. For LI.FI quotes, use the wallet from **defi_get_wallet** as `fromAddress`; execute with **defi_send_transaction** or **defi_approve_and_send**.
